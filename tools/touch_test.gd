...
func _run() -> void:
	...
	# And no PLATES, because there is no plating up on a rope - a button that
	# could only ever be refused is worse than no button.
	_check("PLATES is not offered on a cable", not riding.has(&"shield"))
