...
try {
	...
	foreach ($n in 1, 2) {
		$c = Start-Process -FilePath $godot -PassThru -NoNewWindow `
			-ArgumentList '--headless', '--path', $project, `
				'--script', 'res://tools/fall_net_test.gd', '--', "--peer=$n", '--port=27787' `
	...
	Write-Host ''
	if ($failed -eq 0) { Write-Host 'PASS - the landing carried exactly as far as it should' }
