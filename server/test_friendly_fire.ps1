# Two duos-paired peers try to hurt each other and fail - see
# tools/friendly_fire_test.gd for what this actually checks.
#
#   powershell -File server\test_friendly_fire.ps1

$ErrorActionPreference = 'Stop'

$godot = 'C:\Users\Computer\Downloads\Godot_v4.7.1-stable_win64.exe\Godot_v4.7.1-stable_win64_console.exe'
$project = Split-Path -Parent $PSScriptRoot
$out = Join-Path $env:TEMP 'raid-friendly-fire-test'
...
try {
	...
	$c1 = Start-Process -FilePath $godot -PassThru -NoNewWindow `
		-ArgumentList '--headless', '--path', $project, `
			'--script', 'res://tools/friendly_fire_test.gd', '--', '--peer=1', '--port=27786' `
	...
	$c2 = Start-Process -FilePath $godot -PassThru -NoNewWindow `
		-ArgumentList '--headless', '--path', $project, `
			'--script', 'res://tools/friendly_fire_test.gd', '--', '--peer=2', '--port=27786' `
	...
	Write-Host ''
	if ($failed -eq 0) { Write-Host 'PASS - no friendly fire' }
